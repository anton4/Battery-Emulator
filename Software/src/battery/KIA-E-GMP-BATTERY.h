...
class KiaEGmpBattery : public UdsCanBattery {
 public:
   ...
   * BMS actually needs to keep contactors closed. */
  static constexpr uint32_t enabled_groups = 0xFFFFFFFF;
 ...
 private:
  ...
  void transmit_bus_emulation(unsigned long currentMillis);
  ...
  uint32_t opTime = 0;
  uint32_t cumulative_charge_current_dAh = 0;     // 0.1 Ah
  uint32_t cumulative_discharge_current_dAh = 0;  // 0.1 Ah
  uint32_t cumulative_charge_energy_hWh = 0;      // 0.1 kWh
  uint32_t cumulative_discharge_energy_hWh = 0;   // 0.1 kWh
  ...
  /* Bus emulation state, one slot per EGMP_TX_TABLE entry. The BMS only keeps
   * its contactors closed while it hears the whole vehicle (VCU, motor
   * controllers, charger, compressor, coolant valve ...) with valid CRCs and
   * alive counters, so every non-BMS frame from a real car is replayed. */
  struct TxState {
